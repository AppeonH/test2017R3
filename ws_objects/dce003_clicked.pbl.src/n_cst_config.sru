...
If as_type = '1' Then
	for li_i = li_len to 1 step -1
		ls_word = MidA(as_key,li_i,1)
		ls_key += String(CharA(AscA(ls_word) + li_i))
		as_key = MidA(as_key,1,li_i - 1)
	next	
End If
...
//Get File Name
if PosA(as_filename,'\') > 0 then	
	ls_name = MidA(as_filename,LastPosA(as_filename,'\') + 1)
else
	ls_name = as_filename
end if
